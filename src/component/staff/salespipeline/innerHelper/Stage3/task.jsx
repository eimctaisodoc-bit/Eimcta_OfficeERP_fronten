...
export const PMeeting = ({ id }) => {
    ...
    const validateAll = useCallback(() => {
        ...
        if (isCompleted) {
            ...
            if (!getTrimmedValue(taskForm.meetingLocation)) {
                newErrors.meetingLocation = 'Please enter meeting location.';
                isValid = false;
            }
    ...
    const handleSaveTask = useCallback(() => {
        ...
        if (!validation.isValid) {
            ...
            setTimeout(() => {
                ...
                } else if (validation.errors.meetingFrom || validation.errors.meetingTo || validation.errors.meetingLocation || validation.errors.handledBy) {
        ...
        if (isCompleted) {
            payload.GeneralDetails = {
                ...
                meetingDetails: {
                    ...
                    meetingLocation: getTrimmedValue(taskForm.meetingLocation),
                ...
                <div className={`space-y-4 ${isCancelled ? 'opacity-60 pointer-events-none' : ''}`} ref={meetingRef}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <input
                                type="text"
                                value={isCancelled ? "" : taskForm.meetingLocation}
                                onChange={(e) => updateForm('meetingLocation', e.target.value)}
                                placeholder="Enter meeting location..."
                                className={getInputClass(!!errors.meetingLocation, isCancelled)}
                                disabled={isCancelled}
                            />
                            {errors.meetingLocation && (
                                <div className="flex items-center gap-1.5 text-xs text-red-600 bg-red-50 p-2 rounded">
                                    <AlertCircle size={13} className="flex-shrink-0" />
                                    <span>{errors.meetingLocation}</span>
                                </div>
                            )}
                        </div>
